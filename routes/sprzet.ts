...
router.get('/', async (req, res, next) => {
  ...
  if(accountType === 'KIEROWNIK')  {
    data = await Database.raw("SELECT s.id, s.rodzaj_sprzetu, s.przeznaczenie, s.cecha_1_label,s.cecha_1_value, s.cecha_2_label,s.cecha_2_value, s.cecha_3_label,s.cecha_3_value, s.cecha_4_label,s.cecha_4_value, s.cena_wypozyczenia_dzien FROM sprzet s JOIN rodzaj_sprzetu r ON (s.rodzaj_sprzetu=r.nazwa) WHERE s.blokada!=\"usuniety\" AND r.rodzaj_sezonu=? LIMIT ? OFFSET ?;", [sezon, limit, offset]);
    totalRowsData = await Database.raw("SELECT COUNT(*) as totalRows FROM sprzet s JOIN rodzaj_sprzetu r ON (s.rodzaj_sprzetu=r.nazwa) WHERE s.blokada!=\"usuniety\" AND r.rodzaj_sezonu=?;", [sezon]);
  } else if(accountType === 'SERWISANT') {
    data = await Database.raw("SELECT s.id, s.rodzaj_sprzetu, s.przeznaczenie, s.cecha_1_label,s.cecha_1_value, s.cecha_2_label,s.cecha_2_value, s.cecha_3_label,s.cecha_3_value, s.cecha_4_label,s.cecha_4_value, s.cena_wypozyczenia_dzien FROM sprzet s JOIN rodzaj_sprzetu r ON s.rodzaj_sprzetu=r.nazwa JOIN wypozyczony_sprzet wp ON wp.sprzet_id = s.id JOIN wypozyczenie w ON w.id = wp.wypozyczenie_id AND w.koniec > (SELECT MAX(n.data) FROM naprawa n WHERE s.id = n.sprzet_id) WHERE r.rodzaj_sezonu=? GROUP BY s.id HAVING COUNT(w.id) > 4 LIMIT ? OFFSET ?;", [sezon, limit, offset]);
    totalRowsData = await Database.raw("SELECT COUNT(*) AS totalRows FROM (SELECT COUNT(s.id) as totalRows FROM sprzet s JOIN rodzaj_sprzetu r ON s.rodzaj_sprzetu=r.nazwa JOIN wypozyczony_sprzet wp ON wp.sprzet_id = s.id JOIN wypozyczenie w ON w.id = wp.wypozyczenie_id AND w.koniec > (SELECT MAX(n.data) FROM naprawa n WHERE s.id = n.sprzet_id) WHERE r.rodzaj_sezonu=? GROUP BY s.id HAVING COUNT(w.id) > 4) x", [sezon]);
  } else {    // KLIENT
